...
static void pimoroni_pim447_work_handler(struct k_work *work) {
    ...
    ret = i2c_burst_read_dt(&pimoroni_pim447_i2c, REG_LEFT, buf, 5);
    if (ret) {
    ...
    uint8_t zero = 0;
    ret = i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_LEFT, zero);
    ret |= i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_RIGHT, zero);
    ret |= i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_UP, zero);
    ret |= i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_DOWN, zero);
    ...
    uint8_t int_status;
    ret = i2c_reg_read_byte_dt(&pimoroni_pim447_i2c, REG_INT, &int_status);
    if (ret) {
    ...
    if (int_status & MSK_INT_TRIGGERED) {
        ...
        ret = i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_INT, int_status);
        if (ret) {
...
static int pimoroni_pim447_enable_interrupt(const struct pimoroni_pim447_config *config, bool enable) {
    ...
    ret = i2c_reg_read_byte_dt(&pimoroni_pim447_i2c, REG_INT, &int_reg);
    if (ret) {
    ...
    ret = i2c_reg_write_byte_dt(&pimoroni_pim447_i2c, REG_INT, int_reg);
    if (ret) {
...
static int pimoroni_pim447_init(const struct device *dev) {
    ...
    if (!device_is_ready(pimoroni_pim447_i2c.bus)) {
    ...
    ret = i2c_reg_read_byte_dt(&pimoroni_pim447_i2c, REG_CHIP_ID_L, &chip_id_l);
    if (ret) {
        LOG_ERR("Failed to read chip ID low byte");
        return ret;
    }

    ret = i2c_reg_read_byte_dt(&pimoroni_pim447_i2c, REG_CHIP_ID_H, &chip_id_h);
    if (ret) {
